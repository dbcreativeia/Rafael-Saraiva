...
interface MaterialCampanhaTabProps {
  refreshTrigger?: number;
}

export const MaterialCampanhaTab: React.FC<MaterialCampanhaTabProps> = ({ refreshTrigger }) => {
  ...
  const deleteBatchMaterial = async (ids: string[]) => {
    try {
      const res = await fetch('/api/material/batch-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids })
      });
      if (!res.ok) {
        await Promise.all(ids.map(id => fetch('/api/material/' + id, { method: 'DELETE' })));
      }
  ...
  const uniqueCities = useMemo(() => {
    return Array.from(new Set(materials.map(m => m.cidade))).filter(Boolean).sort();
  }, [materials]);

  // Separação de Impressos e Digitais para detecção precisa de duplicados
  const impressoItems = useMemo(() => materials.filter(m => m.tipoMaterial === 'impresso'), [materials]);
  const digitalItems = useMemo(() => materials.filter(m => m.tipoMaterial === 'digital'), [materials]);
  ...
  }, [materials, cidadeFilter, tipoFilter, search]);
  ...
  const exportFilteredToExcel = (tipo?: 'impresso' | 'digital') => {
    ...
    let filename = "pedidos_material_campanha_TODOS.xlsx";

    if (tipo === 'impresso') {
      dataset = filteredData.filter(m => m.tipoMaterial === 'impresso');
      sheetName = "Impressos";
      filename = "pedidos_material_campanha_IMPRESSO.xlsx";
    } else if (tipo === 'digital') {
      dataset = filteredData.filter(m => m.tipoMaterial === 'digital');
      sheetName = "Digitais";
      filename = "pedidos_material_campanha_DIGITAL.xlsx";
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {totalDuplicatesGeneral > 0 && (
        <div className="bg-gradient-to-r from-amber-500/15 via-orange-500/10 to-red-500/10 border-2 border-amber-400/80 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900">
                Registros Duplicados Detectados em Material de Campanha
              </h4>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-[11px] text-gray-400 font-medium">Campanha geral</p>
        </div>
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-2xl sm:text-3xl font-black text-emerald-600">{uniqueCities.length}</p>
          <p className="text-[11px] text-gray-400 font-medium">Municípios com pedidos</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-gray-100 shadow-sm">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
          ...
          {/* Lado Esquerdo: Filtros de Tipo e Duplicados */}
          ...
          <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2.5">
            ...
            <div className="relative flex-1 sm:w-56 min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600/20 outline-none text-xs sm:text-sm bg-white font-medium text-gray-800"
              />
            </div>
            ...
            <div className="relative flex-1 sm:flex-initial sm:w-48 min-w-[170px]">
              ...
              <select
                ...
                className="w-full pl-9 pr-8 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600/20 outline-none bg-white font-medium text-gray-700 text-xs sm:text-sm cursor-pointer"
              >
      ...
      {tipoFilter === 'duplicados_impresso' && (
        <DuplicateSection
          title="Gerenciamento de Duplicados - Material Impresso"
          ...
          onDeleteSingle={deleteMaterial}
          onDeleteBatch={deleteBatchMaterial}
      ...
      {tipoFilter === 'duplicados_digital' && (
        <DuplicateSection
          title="Gerenciamento de Duplicados - Material Digital"
        ...
        <>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-gray-50/50">
              <div>
                <h3 className="text-base sm:text-lg font-black uppercase text-dark">Lista de Pedidos de Material de Campanha</h3>
